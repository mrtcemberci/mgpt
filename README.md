...
### 2. Supervised Conversational Fine-Tuning (SFT) on Empathetic Multi-Turn Dialogues
To transform a completed base model checkpoint (`tinystories_shard23.bin`) into a conversational assistant (`mgpt_chat.bin`), fine-tune on a dialogue text file (`empathetic_dialogues.txt`) formatted with explicit turn markers (`User: ... \n Assistant: ... \n <|endoftext|>`).
...
# 2. Fine-tune on empathetic_dialogues.txt for 3,000 steps using --resume with a fresh Cosine LR curve (--start-step=0 --total-steps=3000)
.\build_release\Release\mgpt.exe -t -g --resume -d="empathetic_dialogues.txt" -f="mgpt_chat.bin" --vocab-file="tinystories_slice.txt.vocab.bin" --lr=0.00005 -s=3000 --start-step=0 --total-steps=3000 -l=12 -c=384 -w=256 -b=16 -a=2
...
.\build_release\Release\mgpt.exe -t -g -d="math_dataset.txt" --vocab-file="math_dataset.txt.vocab.bin" -f="mgpt_math.bin" -l=12 -c=384 -w=128 -b=16 -a=2 -s=12000 --lr=0.0001

# 2. Test arithmetic reasoning with the trained model
.\build\Release\mgpt.exe -i -g -f="mgpt_math.bin" --vocab-file="math_dataset.txt.vocab.bin" -l=12 -c=384 -w=128 -p="? 35 * 33`n{" --temp=0.1 --topk=5
```